...
/*   Updated: 2025/02/05 21:54:38 by zkhojazo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	// // you should pass through collectibles
	if (*(*(map + r) + c) == '0' || (*(*(map + r) + c) == 'P') || (*(*(map + r) + c) == 'C') || (*(*(map + r) + c) == 'E'))
	{
		if (*(*(map + r) + c) == 'P')
			(*chars).pos++;
		else if (*(*(map + r) + c) == 'E')
...
char	**sl_strdup_double(char **str)
{
	...
	while (*(str + i))
	{
		*(str_cp + i) = ft_strdup(*(str + i)); // maybe you want to handle malloc failure and errno?
		i++;
	}
	*(str_cp + i) = NULL;
	return (str_cp);
